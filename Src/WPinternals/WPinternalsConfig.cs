// WPinternalsConfig & Registration
        ...
        internal static void CheckExpiration()
        {
            ...
            if (DateTime.Now >= ExpirationDate)
            {
                if (Environment.GetCommandLineArgs().Count() > 1)
                {
                    ...
                    MessageBox.Show("This prerelease version is expired!", 
                    "Windows Phone Internals", MessageBoxButton.OK,
    ...
    public class WPinternalsConfig
    {
        internal static WPinternalsConfig ReadConfig()
        {
            ...
            string FilePath = Environment.ExpandEnvironmentVariables(
                "%ALLUSERSPROFILE%\\WPInternals\\WPInternals.config");
        ...
        {
            string DirPath = Environment.ExpandEnvironmentVariables(
                "%ALLUSERSPROFILE%\\WPInternals");
        ...
        (
            ...
            UInt32 FillSize, 
            UInt32 HeaderSize, 
            bool AssumeImageHeaderFallsInGap, 
            bool AllocateAsyncBuffersOnPhone)
        {
        ...
        {
            return FlashProfiles.Where
            (   
                p => 
                (
                  (string.Compare(p.PlatformID, PlatformID, true) == 0) 
                  && (string.Compare(p.PhoneFirmware, PhoneFirmware, true) == 0) 
                  && ( (FfuFirmware == null) || (string.Compare(p.FfuFirmware, FfuFirmware, true) == 0) ) 
                )
            ).FirstOrDefault();
        }
        ...
        internal void AddFfuToRepository(string FFUPath)
        {
            try
            {
                ...
                AddFfuToRepository(FFUPath, NewFFU.PlatformID, NewFFU.GetFirmwareVersion(),
                    NewFFU.GetOSVersion());
            }
            catch (Exception Ex)
            {
        ...
        internal void AddFfuToRepository(string FFUPath, string PlatformID, string FirmwareVersion, 
            string OSVersion)
        {
            FFUEntry Entry = FFURepository.Where
            (
                e => 
                (
                  (e.PlatformID == PlatformID) 
                  && (e.FirmwareVersion == FirmwareVersion) && (string.Compare(e.Path, FFUPath, true) == 0)
                )
            ).FirstOrDefault();
            if (Entry == null)
            {
            ...
            {
                LogFile.Log("FFU not added, because it was already present in the repository.",
        ...
        internal void RemoveFfuFromRepository(string FFUPath)
        {
            ...
            if (Count == 0)
                LogFile.Log("FFU was not removed from repository because it was not present.", 
                    LogType.FileAndConsole);
            else
            {
        ...
        {
            EmergencyFileEntry Entry = EmergencyRepository.Where(e => ((e.Type == Type)
            && (string.Compare(e.ProgrammerPath, ProgrammerPath, true) == 0)))
                .FirstOrDefault();

            if ((Entry != null) && (PayloadPath != null) && (string.Compare(Entry.PayloadPath, 
                PayloadPath, true) != 0))
            {
                LogFile.Log("Updating emergency payload path in repository: "
                    + PayloadPath, LogType.FileAndConsole);
                Entry.PayloadPath = PayloadPath;
                WriteConfig();
            }
            else if (Entry == null)
            {
                LogFile.Log("Adding emergency files to repository: "
                    + ProgrammerPath, LogType.FileAndConsole);
            ...
            {
                Debug.WriteLine("[info] Emergency files not added," +
                    " because they were already present in the repository.");
                LogFile.Log(
                    "Emergency files not added, because they were already present in the repository.",
                    LogType.FileAndConsole);
            }
        }

        // RemoveEmergencyFromRepository
        internal void RemoveEmergencyFromRepository(string ProgrammerPath)
        {
            int Count = 0;
            EmergencyRepository.Where
            (
               e => (string.Compare(e.ProgrammerPath, ProgrammerPath, true) == 0)
            )
            .ToList().ForEach
            (
            e =>
            ...
            if (Count == 0)
            {
                Debug.WriteLine("[info] " +
                    "Emergency file was not removed from repository because it was not present.");
                LogFile.Log("Emergency file was not removed from repository because it was not present.", 
                    LogType.FileAndConsole);
            }
            else
            {
                Debug.WriteLine("[info] " + "Removed Emergency file from repository: " + ProgrammerPath);
                LogFile.Log("Removed Emergency file from repository: "
                    + ProgrammerPath, LogType.FileAndConsole);
                WriteConfig();
            }
