// SwitchModeViewModel (ContextViewModel)
    ...
    internal class SwitchModeViewModel: ContextViewModel
    {
            ...
            : base()
        {
            if ((PhoneNotifier.CurrentInterface != PhoneInterfaces.Lumia_Flash)
                && (PhoneNotifier.CurrentInterface != PhoneInterfaces.Lumia_Bootloader)
                && (PhoneNotifier.CurrentInterface != PhoneInterfaces.Lumia_Label)
                && (PhoneNotifier.CurrentInterface != PhoneInterfaces.Lumia_Normal))
            {
                Debug.WriteLine("[i] SwitchModeViewModel: " + "Argument Exception!");
            ...
            {
                Debug.WriteLine("[i] Waiting for phone to connect...");
        ...
        internal SwitchModeViewModel
        (
            PhoneNotifierViewModel PhoneNotifier, 
            PhoneInterfaces? TargetMode, 
            ModeSwitchProgressHandler ModeSwitchProgress, 
            ModeSwitchErrorHandler ModeSwitchError, 
            ModeSwitchSuccessHandler ModeSwitchSuccess, 
            SetWorkingStatus SetWorkingStatus = null, 
            UpdateWorkingStatus UpdateWorkingStatus = null
        )  : base()
        {
            if ((PhoneNotifier.CurrentInterface == PhoneInterfaces.Lumia_Bootloader)
                && (TargetMode == PhoneInterfaces.Lumia_Flash))
            {
                PhoneInfo Info = default;

                try
                {
                    Info = ((NokiaFlashModel)PhoneNotifier.CurrentModel).ReadPhoneInfo(false);
                }
                catch (Exception ex)
                {
                    Debug.WriteLine("[ex] SwitchModeViewModel - ReadPhoneInfo error: " 
                        + ex.Message);
                }
            ...
            else
            {
                ...
                if (this.CurrentMode == null)
                {
                    Debug.WriteLine("[info] Waiting for phone to connect...");
                    LogFile.Log("Waiting for phone to connect...", LogType.FileAndConsole);
                    PhoneNotifier.NewDeviceArrived += NewDeviceArrived;
                }
                else
        ...
        private void ModeSwitchSuccessWrapper()
        {
            IsSwitching = false;
            if ((UIContext == null) || (SynchronizationContext.Current == UIContext))
                ModeSwitchSuccess((IDisposable)PhoneNotifier.CurrentModel,
                    (PhoneInterfaces)PhoneNotifier.CurrentInterface);
            else
                UIContext.Post(s => 
                { 
                    ModeSwitchSuccess( (IDisposable)PhoneNotifier.CurrentModel, 
        ...
        internal void StartSwitch()
        {
            ...
            switch (CurrentMode)
            {
                ...
                case PhoneInterfaces.Lumia_Label:
                    ...
                    switch (TargetMode)
                    {
                        ...
                        case PhoneInterfaces.Lumia_MassStorage:
                            ...
                            LogFile.Log("First rebooting phone to Flash mode (then attempt Mass Storage mode)",
                                LogType.FileAndConsole);
                            break;
                        case PhoneInterfaces.Qualcomm_Download: // Only available in new Lumia's
                            ...
                            LogFile.Log("First rebooting phone to Flash mode " +
                                "(then attempt Qualcomm Download mode", LogType.FileAndConsole);
                            break;
                        default:
                            return;
                    }
                ...
                case PhoneInterfaces.Lumia_Flash:
                case PhoneInterfaces.Lumia_Bootloader:
                    byte[] BootModeFlagCommand = new byte[] 
                    { 0x4E, 0x4F, 0x4B, 0x58, 0x46, 0x57, 0x00, 0x55, 0x42,
                        0x46, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00 }; // NOKFW UBF
                    ...
                    switch (TargetMode)
                    {
                        ...
                        case PhoneInterfaces.Qualcomm_Download:
                            byte[] RebootToQualcommDownloadCommand = new byte[] { 0x4E, 0x4F, 0x4B,
                                0x58, 0x43, 0x42, 0x45 }; // NOKXCBE

                            RebootCommandResult = ((NokiaPhoneModel)CurrentModel)
                                .ExecuteRawMethod(RebootToQualcommDownloadCommand);
        ...
        private void NewDeviceArrived(ArrivalEventArgs Args)
        {
                ...
                && (TargetMode == PhoneInterfaces.Lumia_Flash))
            {
                try
                {
                    ...
                    // ResetPhoneToFlashMode() reboots to real flash app. Phone will charge.
                    // Works when in BootMgrApp, not when already in FlashApp.
            ...
            else if ((CurrentMode == PhoneInterfaces.Lumia_Bootloader) 
                && (TargetMode == PhoneInterfaces.Lumia_Normal))
            {
                // Do nothing, because booting to Normal shortly goes into Flash mode too.
                // Just wait to arrive in Normal mode;
                PhoneNotifier.NewDeviceArrived += NewDeviceArrived;
            }
            else if ((CurrentMode == PhoneInterfaces.Lumia_Flash)
                && (TargetMode == PhoneInterfaces.Lumia_MassStorage))
            {
                SwitchFromFlashToMassStorageMode(Continuation: true);
            }
            else if ((CurrentMode == PhoneInterfaces.Lumia_Flash) 
                && (TargetMode == PhoneInterfaces.Qualcomm_Download))
            {
                byte[] RebootCommand = new byte[] { 0x4E, 0x4F, 0x4B, 0x52 };
                byte[] RebootToQualcommDownloadCommand = new byte[]
                { 0x4E, 0x4F, 0x4B, 0x58, 0x43, 0x42, 0x45 }; // NOKXCBE

                IsSwitchingInterface = true;
                LogFile.Log("Sending command for rebooting to Emergency Download mode");

                byte[] RebootCommandResult = ((NokiaPhoneModel)CurrentModel)
                    .ExecuteRawMethod(RebootToQualcommDownloadCommand);
        ...
        private void SwitchFromFlashToMassStorageMode(bool Continuation = false)
        {
            ...
            if (Info.FlashAppProtocolVersionMajor < 2)
                MassStorageWarning = "Switching to Mass Storage mode should take about 10 seconds. " +
                    "The phone should be unlocked using an Engineering SBL3 to enable Mass Storage mode. " +
                    "When you unlocked the bootloader, but you did not use an Engineering SBL3, " +
                    "an attempt to boot to Mass Storage mode may result in an unresponsive state." +
                    " Installing drivers for this interface may also cause to hang the PC. So when this switch is taking too long, you should reboot both the PC and the phone. To reboot the phone, you have to perform a soft-reset. Press and hold the volume-down-button and the power-button at the same time for at least 10 seconds. This will trigger a power-cycle and the phone will reboot. Once fully booted, the phone may show strange behavior, like complaining about mail-accounts, showing old text-messages, inability to load https-websites, etc. This is expected behavior, because the time-settings of the phone are incorrect. Just wait a few seconds for the phone to get a data-connection and have the date/time synced. After that the strange behavior will stop automatically and normal operation is resumed.";
            else
            {
                MassStorageWarning = "When the screen of the phone is black for a while, " +
                    "it could be that the phone is already in Mass Storage Mode, " +
                    "but there is no drive-letter assigned. To resolve this issue, " +
                    "open Device Manager and manually assign a drive-letter to the MainOS partition" +
                    " of your phone, or open a command-prompt and type: diskpart automount enable.";

                if (App.IsPnPEventLogMissing)
                    MassStorageWarning += " It is also possible that the phone is in Mass Storage mode, " +
                        "but the Mass Storage driver on this PC failed. " +
                        "Your PC does not have an eventlog to detect this misbehaviour. " +
                        "But in this case you will see a device with an exclamation mark " +
                        "in Device Manager and then you need to manually reset the phone " +
                        "by pressing and holding the power-button for at least 10 seconds " +
                        "until it vibrates and reboots. " +
                        "After that Windows Phone Internals will revert the changes. " +
                        "After the phone has rebooted to the OS, you can retry to unlock the bootloader.";
            }
            ...
            if (IsNewLumia)
            {
                ...
                IsUnlockedNew = ((GPT.GetPartition("IS_UNLOCKED") != null) 
                    || (GPT.GetPartition("BACKUP_EFIESP") != null));
            }

            bool IsOriginalEngineeringLumia = ((!Info.SecureFfuEnabled 
                || Info.Authenticated || Info.RdcPresent) && !IsUnlockedNew);

            if (IsOldLumia || IsOriginalEngineeringLumia)
            {
                byte[] BootModeFlagCommand = new byte[] 
                { 0x4E, 0x4F, 0x4B, 0x58, 0x46, 0x57, 0x00, 0x55, 0x42, 0x46, 0x00, 0x00, 
                    0x00, 0x00, 0x01, 0x00 }; // NOKFW UBF
                byte[] RebootCommand = new byte[] { 0x4E, 0x4F, 0x4B, 0x52 };
                byte[] RebootToMassStorageCommand = new byte[] { 0x4E, 0x4F, 0x4B, 0x4D }; // NOKM
                IsSwitchingInterface = true;
                byte[] RebootCommandResult = ((NokiaPhoneModel)CurrentModel)
                    .ExecuteRawMethod(RebootToMassStorageCommand);

                if ((RebootCommandResult != null) && (RebootCommandResult.Length == 4)) // This means fail: NOKU (unknow command)
                {
                    BootModeFlagCommand[0x0F] = 0x4D;
                    byte[] BootFlagResult = ((NokiaPhoneModel)CurrentModel)
                        .ExecuteRawMethod(BootModeFlagCommand);
                ...
                new Thread(async () =>
                {
                    ...
                    try
                    {
                        ...
                        using (var stream = assembly.GetManifestResourceStream("WPinternals.SBMSM"))
                        {
                            ...
                            {
                                using (System.IO.MemoryStream SB = new System.IO.MemoryStream()) 
                                // Must be a seekable stream!
                                {
                                    dec.CopyTo(SB);
                                    ...
                                    // We've been reading the GPT, so we let the phone reset once more
                                    // to be sure that memory maps are the same
                                    ...
                                    Part.Stream = SB;
                                    Parts.Add(Part);
                                    await LumiaV2UnlockBootViewModel.LumiaV2CustomFlash(PhoneNotifier, 
                                        null, false, false, Parts, DoResetFirst: true, 
                                        ClearFlashingStatusAtEnd: false, ShowProgress: false, 
                                        SetWorkingStatus: (m, s, v, a, st) => 
                                        {
                                            if (SetWorkingStatus != null)
                                            {
                                                if ((st == WPinternalsStatus.Scanning) 
                                                || (st == WPinternalsStatus.WaitingForManualReset))
                                                    SetWorkingStatus(m, s, v, a, st);

                                                else if ((LastStatus == WPinternalsStatus.Scanning) 
                                                || (LastStatus == WPinternalsStatus.WaitingForManualReset))
                        ...
                        if (PhoneNotifier.CurrentInterface == PhoneInterfaces.Lumia_BadMassStorage)
                            throw new WPinternalsException(
                                "Phone is in Mass Storage mode, but the driver on PC failed to start");
            ...
            {
                ModeSwitchErrorWrapper("Bootloader was not unlocked. " +
                    "First unlock bootloader before you try to switch to Mass Storage Mode.");
            }
        }

        internal async static Task<IDisposable> SwitchTo(PhoneNotifierViewModel Notifier, 
            PhoneInterfaces? TargetMode, string RequestedVolumeForMassStorage = "MainOS")
        {
            return await SwitchToWithProgress(Notifier, TargetMode, null, RequestedVolumeForMassStorage);
        }

        internal async static Task<IDisposable> SwitchToWithProgress(
            PhoneNotifierViewModel Notifier, PhoneInterfaces? TargetMode, 
            ModeSwitchProgressHandler ModeSwitchProgress, string RequestedVolumeForMassStorage = "MainOS")
        {
        ...
        internal async static Task<IDisposable> SwitchToWithStatus(PhoneNotifierViewModel Notifier, 
            PhoneInterfaces? TargetMode, SetWorkingStatus SetWorkingStatus = null, 
            UpdateWorkingStatus UpdateWorkingStatus = null, string RequestedVolumeForMassStorage = "MainOS")
        {
            if (Notifier.CurrentInterface == TargetMode)
                return Notifier.CurrentModel;
