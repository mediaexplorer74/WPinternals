...
namespace WPinternals
{
    ...
    internal class PhoneNotifierViewModel
    {
        ...
        private EventWaitHandle NewInterfaceWaitHandle = new EventWaitHandle(
        ...
        internal void Start()
        {
            ...
            try
            {
                EventLogQuery LogQuery = new EventLogQuery(
                    "Microsoft-Windows-Kernel-PnP/Configuration", PathType.LogName,
                    "*[System[(EventID = 411)]]");

                LogWatcher = new EventLogWatcher(LogQuery);
                LogWatcher.EventRecordWritten
                    += new EventHandler<EventRecordWrittenEventArgs>(PnPEventWritten);
            ...
            {
                Debug.WriteLine("[ex] PhoneNotifierViewModel: Start error: " + ex.Message);
            }
        }
        ...
        private void PnPEventWritten(Object obj, EventRecordWrittenEventArgs arg)
        {
            ...
            {
                Debug.WriteLine("[i] Event " + arg.EventRecord.Id.ToString() + ": " + Description);
                LogFile.Log("Event " + arg.EventRecord.Id.ToString() + ": " + Description, LogType.FileOnly);

                Debug.WriteLine("[i] Phone switched to Mass Storage mode, " +
                    "but the driver on the PC did not start correctly");
                LogFile.Log("Phone switched to Mass Storage mode, " +
                    "but the driver on the PC did not start correctly", LogType.FileAndConsole);
        ...
        void LumiaNotifier_Arrival(object sender, USBEvent e)
        {
            try
            {
                ...
                ) // vid_045e & pid_0a00 & mi_03 = Lumia 950 XL normal mode
                {
                    if (((USBNotifier)sender).Guid == OldCombiInterfaceGuid)
                    {
                        ...
                        else
                        {
                            ...
                            // - It is a WP81 / W10M phone, which has an unresponsive
                            // old combi-interface and we need to wait for the new combi-interface to arrive.
                            // We will wait maximum 1 sec for the new interface.
                            // If it doesn't arrive we will start talking on this old interface.
                            // We will start a new thread, because if this thread
                            // is blocked, no new devices will arrive.
                            ...
                            ThreadPool.QueueUserWorkItem(s =>
                            {
                                if (!NewInterfaceWaitHandle.WaitOne(1000))
                                {
                                    ...
                                    LogFile.Log("Found device on interface: " 
                                        + ((USBNotifier)sender).Guid.ToString(), LogType.FileOnly);
                                    LogFile.Log("Device path: " 
                                        + e.DevicePath, LogType.FileOnly);
                                    LogFile.Log("Connected device: Lumia", LogType.FileAndConsole);

                                    LogFile.Log("Mode: Normal", LogType.FileAndConsole);

                                    NewDeviceArrived(
                                        new ArrivalEventArgs((PhoneInterfaces)CurrentInterface, CurrentModel));
                                }
                            });
                        }
                    }
                ...
                (
                  (e.DevicePath.IndexOf("VID_0421&PID_066E", 
                         StringComparison.OrdinalIgnoreCase) >= 0) ||
                    (e.DevicePath.IndexOf("VID_0421&PID_0714", 
                         StringComparison.OrdinalIgnoreCase) >= 0) || // VID_0421&PID_0714 is for Lumia 930
                    (e.DevicePath.IndexOf("VID_045E&PID_0A02", 
                       StringComparison.OrdinalIgnoreCase) >= 0)
                ) // VID_045E&PID_0A02 is for Lumia 950
                {
                ...
                else if ((e.DevicePath.IndexOf(@"disk&ven_qualcomm&prod_mmc_storage",
                    StringComparison.OrdinalIgnoreCase) >= 0) ||
                    (e.DevicePath.IndexOf(@"DISK&VEN_MSFT&PROD_PHONE_MMC_STOR", 
                    StringComparison.OrdinalIgnoreCase) >= 0))
                {
#if DEBUG
                            ...
                            if (!(CurrentModel is MassStorage))
                            {
                                ...
                                if (NewModel.Drive != null) // When logical drive is already known, we use this model. Or else we wait for the logical drive to arrive.
                                {
                                    ...
                                    LogFile.Log("Found device on interface: " 
                                        + ((USBNotifier)sender).Guid.ToString(), LogType.FileOnly);
                                    LogFile.Log("Device path: " + e.DevicePath, LogType.FileOnly);
                                    LogFile.Log("Connected device: Lumia", 
                                        LogType.FileAndConsole);
                                    LogFile.Log("Mode: Mass storage mode",
                                        LogType.FileAndConsole);
                ...
                else if ((e.DevicePath.Length == @"\\.\E:".Length) 
                    && (e.DevicePath.StartsWith(@"\\.\")) && (e.DevicePath.EndsWith(":")))
                {
#if DEBUG
                    LogFile.Log("Mass storage arrived: " + e.DevicePath, LogType.FileOnly);
                    LogFile.Log("Start new thread for getting metadata.", LogType.FileOnly);
                    ...
                    ThreadPool.QueueUserWorkItem(s =>
                    {
                        lock (ModelLock)
                        {
                            if (!(CurrentModel is MassStorage))
                            {
                                ...
                                // When logical drive is already known, we use this model.
                                // Or else we wait for the logical drive to arrive.
                                if (NewModel.Drive != null) 
                                {
                                    CurrentInterface = PhoneInterfaces.Lumia_MassStorage;
                                    CurrentModel = NewModel;
                                    LogFile.Log("Found device on interface: " 
                                    ...
                                    NewDeviceArrived(new ArrivalEventArgs((PhoneInterfaces)CurrentInterface, 
                                        CurrentModel));
                                }
                            }
                        }
                ...
                else if (e.DevicePath.IndexOf("VID_05C6&PID_9008", StringComparison.OrdinalIgnoreCase) >= 0)
                {
                    ...
                    else if ((DeviceInfo.BusName == "QHSUSB_ARMPRG") 
                        || ((DeviceInfo.BusName == "") 
                              && (LastInterface == PhoneInterfaces.Qualcomm_Download)))
                    {
                        ...
                        LogFile.Log("Found device on interface: " + ((USBNotifier)sender).Guid.ToString(), 
                            LogType.FileOnly);
                        ...
                        if (DeviceInfo.BusName == "")
                            LogFile.Log(
                                "Driver does not show busname, assume mode: Qualcomm Emergency Flash 9008", 
                                LogType.FileAndConsole);
                        else
        ...
        void LumiaNotifier_Removal(object sender, USBEvent e)
        {
            if (
                ...
                (e.DevicePath.IndexOf(@"disk&ven_qualcomm&prod_mmc_storage", 
                StringComparison.OrdinalIgnoreCase) >= 0) ||
                (e.DevicePath.IndexOf(@"DISK&VEN_MSFT&PROD_PHONE_MMC_STOR", 
                StringComparison.OrdinalIgnoreCase) >= 0)
            ) 
        ...
        internal async Task<IDisposable> WaitForArrival()
        {
            ...
            if (CurrentInterface == null)
            {
                ...
                LogFile.Log("Waiting for phone to connect...",
                    LogType.FileOnly);
            }
        ...
        {
            Debug.WriteLine("[i] Waiting for phone to disconnect>>>");
